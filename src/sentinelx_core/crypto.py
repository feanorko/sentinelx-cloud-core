...
def encrypt_text(text: str, recipient_public_key: X25519PublicKey) -> str:
    ...
    return f"{PREFIX}:{_b64(ephemeral.public_key().public_bytes(serialization.Encoding.Raw, serialization.PublicFormat.Raw))}:{_b64(nonce)}:{_b64(ciphertext)}"
...
def decrypt_command(value: str) -> str:
    return decrypt_text(value, COMMAND_PRIVATE)
